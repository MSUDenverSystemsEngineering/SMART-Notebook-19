...
Try {
	...
	[string]$appScriptDate = '07/20/2019'
	...
	If ($deploymentType -ine 'Uninstall') {
		...
		Show-InstallationWelcome -CloseApps 'smartink-sbsdkproxy,sbwdkservice,smartboardservice,smarthelperservice,smartink,smartnotification' -CheckDiskSpace -PersistPrompt
		...
		# Smartboard automatically updates older versions, so no pre-uninstall is required
		...
		$exitCode = Execute-MSI -Action 'Install' -Path "$dirFiles\SMARTLearningSuite.msi" -Parameters 'PRODUCT_NOTIFICATION=0 CUSTOMER_LOGGING=0 INSTALL_SPU=0 LAUNCH_SYSTEM_MENU=1 /qn'
		# for when the license comes in: NB_PROD_KEY=`"`" ACTIVATE_LICENSE=1
	...
	ElseIf ($deploymentType -ieq 'Uninstall')
	{
		...
		Show-InstallationWelcome -CloseApps 'smartink-sbsdkproxy,sbwdkservice,smartboardservice,smarthelperservice,smartink,smartnotification' -CloseAppsCountdown 60
		...
		# Definitely not confident enough in just searching for 'smart' and deleting some other program
		Remove-MSIApplications -Name "SMART Ink" -PassThru
		Remove-MSIApplications -Name "SMART Notebook" -PassThru
		Remove-MSIApplications -Name "SMART Lesson Activity Toolkit" -PassThru
		Remove-MSIApplications -Name "SMART Learning Suite" -PassThru
		Remove-MSIApplications -Name "SMART Gallery Essentials" -PassThru
		Remove-MSIApplications -Name "SMART Product Drivers" -PassThru
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCDwMSKlwyJhReqJ
# OeeYRSHyVTbrgdwpDnVoV9awwFiMcqCCFFwwggQUMIIC/KADAgECAgsEAAAAAAEv
...
# BgEEAYI3AgELMQ4wDAYKKwYBBAGCNwIBFTAvBgkqhkiG9w0BCQQxIgQgK8h9PlaF
# HljXXj5jnLetzlyrxBPILivW1UmDlqqQ0b8wDQYJKoZIhvcNAQEBBQAEggEAR7/U
# ccqlFHY+2TPNrJHEpubrbG0rVLdhRQe2KKm7wjH+de2mX6bC4Tyav+qk0JZwpW0q
# WR2Qo9nmC2BTyKBQnmpM8ZvnSO3hSewz7bchyfMjHIJM9Ne6MO/ulyHnxyT8RhPw
# NQ5pL8zbt7QuElx61Sd/LkbsH9palTwJIfY2Pyq/pypf5xaH0VnE//8u1H/FQihY
# RCTqpFH6vRZPWMEkmSd+OwfmSvCzJYJW/4goOHZusEazjAay2NUDWKfdbhuPlE2S
# hBSGR0qm/XjAzCYy/i0XAq6lT9cPT4P97KbZeB7yylOsUB9pds0pOGZhcNR6lxLX
# rZMtdlycEmcVy8M65aGCAqIwggKeBgkqhkiG9w0BCQYxggKPMIICiwIBATBoMFIx
...
# CSqGSIb3DQEJBTEPFw0xOTA3MjAxOTQ3MDdaMCMGCSqGSIb3DQEJBDEWBBQrK1Of
# brQJGTDNlEiMg3CAW8iSQzCBnQYLKoZIhvcNAQkQAgwxgY0wgYowgYcwgYQEFGO4
...
# ggEAcBeixTh45alfqF3Erst6sHB1+0UZ+C9dj4+XuIW92UPDMqLGp1AKZDRDGPEZ
# m+j7RHc4//lvtDmqiPbbncQbIZ1P1AjmwfYHrqVg/G70yalOnTthfrylwOnb46hJ
# vmY7YQTUrzy7G6UPDtyAYw9DacnMU2P/9IUSBjdYagildmG2sga7CYhVINKJGfIX
# RoFr6MUwTD259zeXxGvh6AFKIf4kazX3OPWHSozjdzlf+xbOF7Li25zQsEx1XYaX
# 7rYjfW9XHFKHlVNucWEIDrR8enQYXxO327RPpK2XArFymY0cGeI/VGVxHn59TY/m
# H8odzBAA0Gl1eulAQj05U6Rcdw==
